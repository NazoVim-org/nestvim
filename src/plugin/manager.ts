...
import type { EditorEventName, EditorEventPayload, LoadedPlugin } from "./types";
...
export class PluginManager {
  ...
  async loadAll(): Promise<void> {
    ...
    for (const plugin of tsPlugins) {
      try {
        ...
        process.stderr.write(
          `[PluginManager] setup() failed for "${plugin.name}": ${err instanceof Error ? err.message : err}\n`,
        );
      }
    }
    ...
    process.stderr.write(`[plugin] ${this.plugins.length} plugin(s) loaded.\n`);
  }
